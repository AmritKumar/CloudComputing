...
The scheme proposed in \cite{cryptoeprint:2009:571} has 3 parameters:($N, \nu, \mu$ ). The somewhat homomorphic scheme uses 5 algorithms: (\texttt{KeyGen}, \texttt{Encrypt}, \texttt{Decrypt}, \texttt{Add}, \texttt{Mult}). Algorithm \autoref{Code:SV} presents the \texttt{KeyGen} algorithm. This sheme will be referred to as SV scheme in this paper.
...
\textbf{SWHE to FHE :} We recall that the authors have proved that the multiplicative depth $d$ of SWHE  verifies \[d \cdot \mathrm{log}(2) \leq  \mathrm{log}\left( \mathrm{log}\left(\frac{\nu}{2\cdot \sqrt{N}}\right) \right) - \mathrm{log}\left(\mathrm{log}\left( N\mu\right)\right)\]
...
We used the open source implementation Scarab library \cite{hcrypt}  version $1.0.0$ of the Smart-Vercauteren scheme and is a part of \textsc{hcryptProject}. The implementation is in beta version. The software requires GMP(we tested with version $5.1.2$) \cite{gmp}, The GNU Multiple Precision Arithmetic Library; FLINT version $1.6$ \cite{flint} , Fast Library for Number Theory; MPIR (we tested with $2.6.0$) \cite{mpir}, Multiple Precision Integers and Rationals and MPFR (tested with version $3.1.2$) \cite{mpfr}  Multiple-Precision Floating-point computations with correct Rounding. Linux is supposed to be the only supported platform. The libraries are implemented in  C and the installation guide is provided with the package. 

\autoref{funs} presents the list of functions provided by the implementation with their description. The library provides a file \texttt{test.c} where a user can write his own functions to test different operations. It also provides a header file \texttt{parameters.h} to set the parameters $N, \nu, \mu, s_1$ and $ s_2$ of the cryptosystem.
\begin{table*}
...
\textbf{Majority of $n$-bits :} Majority bit is evaluated by obtaining the sum of the bits and then returning the result of the comparision of the sum and $n/2$. This operation is interesting in the sense that if the same program is evaluated on the user's side, he has to store the sum of the bits, hence  needs $\mathrm{\lfloor\log_{2}n \rfloor +1}$ bits. However, if he performs the operation remotely on a cloud, he only requires a constant number of bits to retrieve the result. \newline \newline
...
\includegraphics{/home/kumaram/Bureau/Measurements/PARAMS1/figures/InsertionSort/Insertion\_sort\_min-max\_4bits.jpg}
 \label{fig:insernbit4}
 \caption{Wall time for Insertion sort}
\end{figure}
\end{comment}
 \textbf{Sorting :} 
The experiments on the sorting methods were done on the machine M1. For each test 5 random samples were taken. These were generated using the Unix \texttt{rand} function by setting the seed using \texttt{srand(time(NULL))}. Intially the cryptographic parameters were set to $(N = 8, \nu = 2^{384}, \mu = 4, s_1 = 8, $  and $ s_2 = 5)$which is the default setting in the software. The above figures show how the CPU time and Wall time vary with the increase in the number of sequences to sort and the number of bits. These experiments were done for Insertion Sorting algorithm. We also present a  curve fitting plot that verifies quadratic nature of the complexity of this algorithm.

\begin{table}
\caption{CPU time for Bitonic Sort}
\resizebox{8cm}{!}{
 
\begin{tabular}{|l|l|l||}
  \hline
  (Number of bits,  & Maximum time (in ms) & Minimum time (in ms) \\
  Number of sequences) &  &    \\
  \hline
  (4, 4) & 		 34584 & 32759\\
  (8,4)  & 	  	 189685  & 130157\\
...
\begin{table}
\caption{CPU time for Odd-Even Sort}
\resizebox{8cm}{!}{
...
When we increase the security of the system by setting the parameters to  $(N = 256, \nu = 16, \mu = 2, s_1 = 5000, s_2 = 5)$ which ensures $25$ bits of security, we obtain the following results for Odd-Even Sorting algorithm.
